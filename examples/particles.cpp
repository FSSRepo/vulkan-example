...
public:
    ...
    void init(VulkanInstance& inst, VulkanSwapchain& chain,
        ...
        createCommandPool();
        std::cerr << "PS_INIT: command pool done\n";
        createBuffers();
        std::cerr << "PS_INIT: buffers done\n";
        createComputePipeline(compCode);
        std::cerr << "PS_INIT: compute pipeline done\n";
        createGraphicsDescriptors();
        std::cerr << "PS_INIT: graphics descriptors done\n";
        createCommandBuffers();
        std::cerr << "PS_INIT: command buffers done\n";
        ...
        std::cerr << "PS_INIT: sync done\n";
        std::cerr << "PS_INIT: done\n";
    }
    ...
    void createCommandPool() {
        ...
        poolInfo.queueFamilyIndex = 0; // assume graphics queue family is 0
    ...
    void recordCommandBuffer(uint32_t frame, uint32_t imageIndex, VulkanSwapchain& chain,
        ...
        std::cerr << "PS_RECORD: start frame " << frame << "\n";
        ...
        std::cerr << "PS_RECORD: command buffer began\n";
        ...
        std::cerr << "PS_RECORD: dispatch done\n";
        ...
        vkCmdPipelineBarrier(commandBuffers[frame],
            ...
            1, &barrier,
            0, nullptr);
        std::cerr << "PS_RECORD: barrier done\n";
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan Particles", nullptr, nullptr);

    bool debug_app = true;
    VulkanInstance inst(debug_app);
    inst.attach(window);
    inst.initializeDevice();
    std::cerr << "MAIN: instance initialized\n";

    VulkanSwapchain chain(inst);
    chain.initalize(WIDTH, HEIGHT, false);
    std::cerr << "MAIN: swapchain initialized\n";

    auto compShaderCode = readFile("particles.comp.spv");
    auto vertShaderCode = readFile("particles.vert.spv");
    auto fragShaderCode = readFile("particles.frag.spv");
    std::cerr << "MAIN: shaders loaded\n";
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::cerr << "MAIN: frame start\n";
        particles.render(chain, gpipe, dt, camera);
        std::cerr << "MAIN: frame done\n";
    }
