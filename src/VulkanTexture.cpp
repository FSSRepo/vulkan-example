...
void VulkanTexture::copyBufferToImage(VkBuffer buffer, VkImage vimage, uint32_t w, uint32_t h)
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
    ...
    vkCmdCopyBufferToImage(commandBuffer, buffer, vimage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
{
    imgWidth = static_cast<uint32_t>(w);
    imgHeight = static_cast<uint32_t>(h);

    VkDeviceSize imageSize = static_cast<VkDeviceSize>(imgWidth) * static_cast<VkDeviceSize>(imgHeight) * 4;
    ...
    copyBufferToImage(stagingBuffer, image, imgWidth, imgHeight);
